...
export class JwtAcessService {
    ...
    public async decodeAccessToken(token: string): Promise<AccessTokenPayload> {
        let decodedToken: AccessTokenPayload;

        await jwt.verify(token, process.env.JWT_SECRET, async (err: jwt.VerifyErrors, decoded: jwt.JwtPayload) => {
